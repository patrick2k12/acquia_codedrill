	...
	jQuery.fn.extend({
		...
		revredraw: function(oy) {
					return this.each(function() {
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0) {
							var bt = container.parent().find('.cd-bannertimer');
							var opt = bt.data('opt');
		...
		revnext: function(options) {
					return this.each(function() {
						...
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0)
							container.parent().find('.cd-rightarrow').click();
		...
		revprev: function(options) {
					return this.each(function() {
						...
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0)
							container.parent().find('.cd-leftarrow').click();
					})
		...
		function initSlider(container,opt) {
						...
						if (container.data('aimg')!=undefined) {
							if ((container.data('aie8')=="enabled" && isIE(8)) || (container.data('amobile')=="enabled" && is_mobile()))
								container.html('<img class="cd-slider-alternative-image" src="'+container.data("aimg")+'">');
						}
						...
						if (opt.simplifyAll=="on" && (isIE(8) || iOSVersion())) {
							container.find('.cd-caption').each(function() {
								var tc = jQuery(this);
						...
						if (opt.forceFullWidth=="on" && container.closest('.forcefullwidth_wrapper_tp_banner').length==0) {
							...
							container.closest('.forcefullwidth_wrapper_tp_banner').append('<div class="cd-fullwidth-forcer" style="width:100%;height:'+container.height()+'px"></div>');
						...
						try{
							if (opt.hideThumbsUnderResolution>jQuery(window).width() && opt.hideThumbsUnderResolution!=0) {
								container.parent().find('.cd-bullets.cd-thumbs').css({display:"none"});
							} else {
								container.parent().find('.cd-bullets.cd-thumbs').css({display:"block"});
							}
						} catch(e) {}
						...
						if (!container.hasClass("revslider-initialised")) {
									...
									container.find('.caption').each(function() { jQuery(this).addClass('cd-caption')});
									...
									container.find('.cd-caption').each(function(i) {
									...
									container.find('.cd-caption video').each(function(i) {
									...
									container.append('<div class="cd-loader '+opt.spinner+'">'+
												  		'<div class="dot1"></div>'+
												  	    '<div class="dot2"></div>'+
									...
									// RESET THE TIMER
									if (container.find('.cd-bannertimer').length==0) container.append('<div class="cd-bannertimer" style="visibility:hidden"></div>');
									var bt=container.find('.cd-bannertimer');
									if (bt.length>0) {
										bt.css({'width':'0%'});
									};
									...
									container.addClass("cd-simpleresponsive");
									...
									if (opt.shadow!=0) {
										container.parent().append('<div class="cd-bannershadow cd-shadow'+opt.shadow+'"></div>');
										var loff=0;
										if (opt.forceFullWidth=="on")
													loff = 0-opt.container.parent().offset().left;
										container.parent().find('.cd-bannershadow').css({'width':opt.width,'left':loff});
									}
									...
									********************************/
									...
									try{
										if (opt.hideThumbsUnderResoluition!=0 && opt.navigationType=="thumb") {
											if (opt.hideThumbsUnderResoluition>jQuery(window).width())
												jQuery('.cd-bullets').css({display:"none"});
											 else
											 	jQuery('.cd-bullets').css({display:"block"});
										}
									} catch(e) {}
									...
									//if (opt.fullScreen=="on") {
										container.find('.cd-scrollbelowslider').on('click',function() {
												var off=0;
												try{
...
********************************/
		...
		********************************/
		...
		var createBullets = function(container,opt) {
			...
			if (opt.navigationType=="bullet"  || opt.navigationType=="both") {
						container.parent().append('<div class="cd-bullets '+starthidebullets+' simplebullets '+opt.navigationStyle+'"></div>');
			}

			var bullets = container.parent().find('.cd-bullets');
		...
		var createArrows = function(container,opt) {
						var bullets = container.find('.cd-bullets'),
							hidden="",
						...
						function aArrow(dir) {
							container.parent().append('<div style="'+hidden+'" class="cd-'+dir+'arrow '+starthidearrows+' tparrows '+arst+'"><div class="cd-arr-allwrapper"><div class="cd-arr-iwrapper"><div class="cd-arr-imgholder"></div><div class="cd-arr-imgholder2"></div><div class="cd-arr-titleholder"></div><div class="cd-arr-subtitleholder"></div></div></div></div>');
						}
						aArrow("left");
						aArrow("right");

						// 	THE LEFT / RIGHT BUTTON CLICK !	 //
						container.parent().find('.cd-rightarrow').click(function() {
							if (opt.transition==0) {
						...
						container.parent().find('.cd-leftarrow').click(function() {
							if (opt.transition==0) {
									opt.next = opt.next-1;
		...
		var hideThumbs = function(container,opt) {
			...
			if (bullets==null) {
				container.append('<div class=".cd-bullets"></div>');
				var bullets = container.parent().find('.cd-bullets');
			}
		...
		var setBulPos = function(container,opt) {
			...
			var bullets=topcont.find('.cd-bullets');
			...
			var tl = topcont.find('.cd-leftarrow');
			var tr = topcont.find('.cd-rightarrow');
		...
		*******************************************************/
		...
		var handleSpecialPreviews = function(opt) {
			...
			if (opt.arr == undefined) {
				...
				opt.rar = container.parent().find('.cd-rightarrow'),
				opt.lar = container.parent().find('.cd-leftarrow'),
				opt.raimg = opt.rar.find('.cd-arr-imgholder'),
				opt.laimg = opt.lar.find('.cd-arr-imgholder'),
				opt.raimg_b = opt.rar.find('.cd-arr-imgholder2'),
				opt.laimg_b = opt.lar.find('.cd-arr-imgholder2'),
				opt.ratit = opt.rar.find('.cd-arr-titleholder'),
				opt.latit = opt.lar.find('.cd-arr-titleholder');
			}
			...
			if (arr.hasClass("preview2") && !arr.hasClass("hashoveralready")) {
				...
				if (!is_mobile())
					arr.hover(function() {
						...
						var arr = jQuery(this),
							th = arr.find('.cd-arr-titleholder');
			...
			if (rar.hasClass("preview4") && !rar.hasClass("hashoveralready")) {
				...
				rar.hover(function() {
					var iw = jQuery(this).find('.cd-arr-iwrapper');
					var all = jQuery(this).find('.cd-arr-allwrapper');
		...
		var setSize = function(img,opt) {
				...
				opt.container.closest('.forcefullwidth_wrapper_tp_banner').find('.cd-fullwidth-forcer').css({'height':opt.container.height()});
		...
		var prepareSlides = function(container,opt) {
			...
			container.find('.cd-caption').each(function() { jQuery(this).addClass(jQuery(this).data('transition')); jQuery(this).addClass('start') });

			// PREPARE THE UL CONTAINER TO HAVEING MAX HEIGHT AND HEIGHT FOR ANY SITUATION
			container.find('>ul:first').css({overflow:'hidden',width:'100%',height:'100%',maxHeight:container.parent().css('maxHeight')}).addClass("cd-revslider-mainul");
			if (opt.autoHeight=="on") {
			...
			container.find('>ul:first >li').each(function(j) {
				var li=jQuery(this);
				li.addClass("cd-revslider-slidesli");
				...
				if (li.data('link')!=undefined) {
					...
					var apptxt = '<div class="cd-caption sft slidelink" style="width:100%;height:100%;z-index:'+zindex+';" data-x="center" data-y="center" data-linktoslide="'+linktoslide+'" data-start="0"><a style="width:100%;height:100%;display:block"';
			...
			container.find('>ul:first >li >img').each(function(j) {
				...
				if (opt.dottedOverlay!="none" && opt.dottedOverlay!=undefined)
						img.closest('.slotholder').append('<div class="cd-dottedoverlay '+opt.dottedOverlay+'"></div>');
				...
				img.replaceWith('<div class="cd-bgimg defaultimg" data-lazyload="'+img.data('lazyload')+'" data-bgfit="'+bgfit+'"data-bgposition="'+bgposition+'" data-bgrepeat="'+bgrepeat+'" data-lazydone="'+img.data('lazydone')+'" src="'+src+'" data-src="'+src+'" style="background-color:'+img.css("backgroundColor")+';background-repeat:'+bgrepeat+';background-image:url('+src+');background-size:'+bgfit+';background-position:'+bgposition+';width:100%;height:100%;"></div>');

				if (isIE(8)) {
					pari.find('.cd-bgimg').css({backgroundImage:"none",'background-image':'none'});
					pari.find('.cd-bgimg').append('<img class="ieeightfallbackimage defaultimg" src="'+src+'" style="width:100%">');
				}
		...
		********************************************/
		...
		var swapSlide = function(container,opt) {
				...
				if (defimg.data('lazyload') !=undefined && defimg.data('lazyload') !="undefined" && defimg.data('lazydone') !=1 ) {
					...
					container.find('.cd-loader').css({display:"block"});
					waitForLoads(container.find('.cd-static-layers'),function() {
		...
		var swapSlideCall = function(opt,defimg,container) {
		    ...
		    container.find('.cd-loader').css({display:"none"});
		    setSize(defimg,opt);
			setBulPos(container,opt);
			setSize(defimg,opt);
		...
		/*	if (opt.ffnn == undefined) opt.ffnn=0;
			...
			*/
			...
			if (nexttrans==16) {						// PAPERCUT
					...
					// PREPARE THE CUTS
					actli.wrapInner('<div class="cd-half-one" style="position:relative; width:100%;height:100%"></div>');

					actli.find('.cd-half-one').clone(true).appendTo(actli).addClass("cd-half-two");
					actli.find('.cd-half-two').removeClass('cd-half-one');
					...
					actli.find('.cd-half-one .defaultimg').wrap('<div class="cd-papercut" style="width:'+oow+'px;height:'+ooh+'px;"></div>')
					actli.find('.cd-half-two .defaultimg').wrap('<div class="cd-papercut" style="width:'+oow+'px;height:'+ooh+'px;"></div>')
					actli.find('.cd-half-two .defaultimg').css({position:'absolute',top:'-50%'});
					actli.find('.cd-half-two .cd-caption').wrapAll('<div style="position:absolute;top:-50%;left:0px;"></div>');

					mtl.add(punchgs.TweenLite.set(actli.find('.cd-half-two'),
					                 {width:oow,height:ooh,overflow:'hidden',zIndex:15,position:'absolute',top:ooh/2,left:'0px',transformPerspective:600,transformOrigin:"center bottom"}),0);

					mtl.add(punchgs.TweenLite.set(actli.find('.cd-half-one'),
					...
					mtl.add(punchgs.TweenLite.set(actli.find('.cd-half-one'),{overflow:'hidden'}),0);
					mtl.add(punchgs.TweenLite.fromTo(actli.find('.cd-half-one'),masterspeed/800,
					                 {width:oow,height:ooh/2,position:'absolute',top:'0px',left:'0px',force3D:"auto",transformOrigin:"center top"},
					                 {scale:sc1,rotation:ro1,y:(0-ooh-ooh/4),autoAlpha:0,ease:punchgs.Power2.easeInOut}),0);
					mtl.add(punchgs.TweenLite.fromTo(actli.find('.cd-half-two'),masterspeed/800,
					                 ...
					                 {scale:sc2,rotation:ro2,y:ooh+ooh/4,ease:punchgs.Power2.easeInOut,autoAlpha:0,onComplete:function() {
										...
										if (actli.find('.cd-half-one').length>0)  {
											actli.find('.cd-half-one .defaultimg').unwrap();
											actli.find('.cd-half-one .slotholder').unwrap();
										}
										actli.find('.cd-half-two').remove();
					                 }}),0);
		...
		**************************************/
		...
		var  onPlayerStateChange = function(event) {
			 ...
			 var container = ytcont.closest('.cd-simpleresponsive');
			...
			if (event.data == YT.PlayerState.PLAYING) {
				...
				if (ytcont.closest('.cd-caption').data('volume')=="mute")
					  player.mute();
		...
		var vimeoready_auto = function(player_id,autoplay) {
			...
			var froogaloop = $f(player_id),
				...
				container = vimcont.closest('.cd-simpleresponsive'),
				nextcaption = vimcont.closest('.cd-caption');
			...
			setTimeout(function() {
				froogaloop.addEvent('ready', function(data) {
						...
						// PLAY VIDEO IF THUMBNAIL HAS BEEN CLICKED
						 nextcaption.find('.cd-thumb-image').click(function() {
			...
			var updateHTML5Size = function(pc,container) {
				...
				var animateTheCaptions = function(nextli, opt,recalled,mtl) {
						...
						var ctl = nextli.data('ctl'),
							offsetx=0,
							offsety=0,
							allcaptions = nextli.find('.cd-caption'),
							allstaticcaptions = opt.container.find('.cd-static-layers').find('.cd-caption');
						...
						allcaptions.each(function(i) {
								...
								if (nextcaption.hasClass("cd-static-layer")) {
									...
									if (!nextcaption.hasClass("cd-is-shown")) {
										...
										if ((nss<=opt.next && nes>=opt.next) ||
											(nss == opt.next) || (nes == opt.next)){
												...
												nextcaption.addClass("cd-is-shown");
												staticdirection = 1;
										} else {
									...
									// IF STATIC ITEM ALREADY VISIBLE
										...
										if ((nes==opt.next) ||
											...
											staticdirection = 2;
											//nextcaption.removeClass("cd-is-shown");
										} else {
											staticdirection = 0;
										}
								...
								if (opt.width<opt.hideCaptionAtLimit && nextcaption.data('captionhidden')=="on") {
									nextcaption.addClass("cd-hidden-caption")
									handlecaption=1;
								} else {
									if (opt.width<opt.hideAllCaptionAtLimit || opt.width<opt.hideAllCaptionAtLilmit)	{
										...
										nextcaption.removeClass("cd-hidden-caption")
									}
								}
								...
								if (handlecaption==0) {
										...
										if (nextcaption.data('linktoslide')!="no") {
											nextcaption.click(function() {
												...
												if (dir!="next" && dir!="prev") {
													...
													opt.container.parent().find('.cd-rightarrow').click();
												} else
													if (dir=="next")
														opt.container.parent().find('.cd-rightarrow').click();
												else
													if (dir=="prev")
														opt.container.parent().find('.cd-leftarrow').click();
											});
										}
									...
									if (nextcaption.hasClass("cd-videolayer") || nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0 ) {
										...
										if ((videomp!=undefined || videowebm!=undefined) && nextcaption.find('video').length==0) {
											...
											if (videocontrols=="controls")
												nextcaption.append('<div class="cd-video-controls">'+
																		'<div class="cd-video-button-wrap"><button type="button" class="cd-video-button cd-vid-play-pause">Play</button></div>'+
																		'<div class="cd-video-seek-bar-wrap"><input  type="range" class="cd-seek-bar" value="0"></div>'+
																		'<div class="cd-video-button-wrap"><button  type="button" class="cd-video-button cd-vid-mute">Mute</button></div>'+
																		'<div class="cd-video-vol-bar-wrap"><input  type="range" class="cd-volume-bar" min="0" max="1" step="0.1" value="1"></div>'+
																		'<div class="cd-video-button-wrap"><button  type="button" class="cd-video-button cd-vid-full-screen">Full-Screen</button></div>'+
																	'</div>');
										}
										...
										nextcaption.find('iframe').each(function() {
												...
												if (nextcaption.data('videoposter')!=undefined && nextcaption.data('videoposter').length>2 && nextcaption.data('autoplay')!=true && !internrecalled) {
													if (nextcaption.find('.cd-thumb-image').length==0)
														nextcaption.append('<div class="cd-thumb-image" style="cursor:pointer; position:absolute;top:0px;left:0px;width:100%;height:100%;background-image:url('+nextcaption.data('videoposter')+'); background-size:cover"></div>');
													else
													  punchgs.TweenLite.set(nextcaption.find('.cd-thumb-image'),{autoAlpha:1});
												}
																				...
																				events: {
																					...
																					'onReady': function(event) {
																						var embedCode = event.target.getVideoEmbedCode(),
																							...
																							nextcaption = ytcont.closest('.cd-caption'),
												...
												html5vid.data('interval',setInterval(function() {
													if (html5vid.data('metaloaded')==1 || video.duration!=NaN) {
														...
														if (!html5vid.hasClass("HasListener")) {
																...
																if (nextcaption.find('.cd-dottedoverlay').length!=1)
																	html5vid.append('<div class="cd-dottedoverlay '+nextcaption.data('dottedoverlay')+'"></div>');

																if (jvideo.attr('control') == undefined ) {
																	if (html5vid.find('.cd-video-play-button').length==0)
																		html5vid.append('<div class="cd-video-play-button"><i class="revicon-right-dir"></i><div class="cd-revstop"></div></div>');
																	html5vid.find('video, .cd-poster, .cd-video-play-button').click(function() {
																...
																var playButton = nextcaption.find('.cd-vid-play-pause')[0],
																	muteButton = nextcaption.find('.cd-vid-mute')[0],
																	fullScreenButton = nextcaption.find('.cd-vid-full-screen')[0],
																	seekBar = nextcaption.find('.cd-seek-bar')[0],
																	volumeBar = nextcaption.find('.cd-volume-bar')[0];
															...
															if (html5vid.closest('.cd-caption').data('forcecover')==1) {
																updateHTML5Size(html5vid,opt.container);
																html5vid.addClass("fullcoveredvideo");
															}
									...
									if (nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0) {
												...
												nextcaption.find('.cd-resizeme, .cd-resizeme *').each(function() {
														calcCaptionResponsive(jQuery(this),opt);
												});

												if (nextcaption.hasClass("cd-resizeme")) {
							...
							if (!internrecalled) {
									...
									if (nextcaption.data('splitin') == "chars" || nextcaption.data('splitin') == "words" || nextcaption.data('splitin') == "lines" || nextcaption.data('splitout') == "chars" || nextcaption.data('splitout') == "words" || nextcaption.data('splitout') == "lines") {
										if (nextcaption.find('a').length>0)
											nextcaption.data('mySplitText',new punchgs.SplitText(nextcaption.find('a'),{type:"lines,words,chars",charsClass:"cd-splitted",wordsClass:"cd-splitted",linesClass:"cd-splitted"}));
										else
										if (nextcaption.find('.cd-layer-inner-rotation').length>0)
											nextcaption.data('mySplitText',new punchgs.SplitText(nextcaption.find('.cd-layer-inner-rotation'),{type:"lines,words,chars",charsClass:"cd-splitted",wordsClass:"cd-splitted",linesClass:"cd-splitted"}));
										else
											nextcaption.data('mySplitText',new punchgs.SplitText(nextcaption,{type:"lines,words,chars",charsClass:"cd-splitted",wordsClass:"cd-splitted",linesClass:"cd-splitted"}));
						...
						var bt=jQuery('body').find('#'+opt.container.attr('id')).find('.cd-bannertimer');
						bt.data('opt',opt);
				...
				********************************/
				...
				var removeTheCaptions = function(actli,opt) {
						...
						var allcaptions = actli.find('.cd-caption'),
							allstaticcaptions = opt.container.find('.cd-static-layers').find('.cd-caption');
		...
		var countDown = function(container,opt) {
			...
			if (opt.slideamount >1 && !(opt.stopAfterLoops==0 && opt.stopAtSlide==1) ) {
					...
					container.on('stoptimer',function() {
						var bt = jQuery(this).find('.cd-bannertimer');
						bt.data('tween').pause();
					...
					container.on('nulltimer',function() {
					 ...
					 var countDownNext = function() {
						...
						if (opt.stopLoop=="on" && opt.next==opt.lastslidetoshow-1) {
								container.find('.cd-bannertimer').css({'visibility':'hidden'});
...
**************************************************************************/
			...
			container.find('>ul:first-child >li').each(function() {
				...
				for (var i = 1; i<=10;i++)
					li.find('.rs-parallaxlevel-'+i).each(function() {
						var pw = jQuery(this);
						pw.wrap('<div style="position:absolute;top:0px;left:0px;width:100%;height:100%;z-index:'+pw.css('zIndex')+'" class="cd-parallax-container" data-parallaxlevel="'+opt.parallaxLevels[i-1]+'"></div>');
					});
			})
						...
						container.on('mousemove.hoverdir, mouseleave.hoverdir',function(event) {
							...
							switch (event.type) {
								...
								case "mousemove":
										...
										currslide.find(".cd-parallax-container").each(function() {
											var pc = jQuery(this),
		...
		***************************************/
		var scrollParallax = function(container,opt) {
			...
			var currslide = container.find('.current-sr-slide-visible');
			container.find(".cd-parallax-container").each(function(i) {
				var pc = jQuery(this),
					pl = parseInt(pc.data('parallaxlevel'),0)/100,
					offsv =	diffv * pl;
		...
		**************************************************************************/
		...
		var createThumbs = function(container,opt) {
			...
			if (opt.navigationType=="thumb" || opt.navsecond=="both") {
						cap.append('<div class="cd-bullets cd-thumbs '+opt.navigationStyle+'"><div class="cd-mask"><div class="cd-thumbcontainer"></div></div></div>');
			}
			var bullets = cap.find('.cd-bullets.cd-thumbs .cd-mask .cd-thumbcontainer');
		...
		var moveSelectedThumb = function(container) {
									...
									var bullets=container.parent().find('.cd-bullets.cd-thumbs .cd-mask .cd-thumbcontainer'),
										$this=bullets.parent(),
										offset = $this.offset(),
										minwidth=$this.find('.bullet:first').outerWidth(true),
		...
		var moveThumbSliderToPosition = function($this,pos,speed) {
			punchgs.TweenLite.to($this.find('.cd-thumbcontainer'),0.2,{force3D:"auto",left:pos,ease:punchgs.Power3.easeOut,overwrite:"auto"});
		}
})(jQuery);
